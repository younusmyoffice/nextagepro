...
class ShopController extends Controller
{
    public function ShopPage(){
        ...
        if (!empty($_GET['category'])) {
            ...
            $products = $products->whereIn('category_id',$catIds)->paginate(33);
        }
         if (!empty($_GET['brand'])) {
            $slugs = explode(',',$_GET['brand']);
            $brandIds = Brand::select('id')->whereIn('brand_slug_en',$slugs)->pluck('id')->toArray();
            $products = $products->whereIn('brand_id',$brandIds)->paginate(33);
        }
        else{
             $products = Product::where('status',1)->orderBy('id','DESC')->paginate(33);
        }
